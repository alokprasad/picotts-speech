...
#include "../include/svox_ssml_parser.h"
#include "../include/TtsEngine.h"
#include "../picotts-src/src/picoapi.h"
#include "../picotts-src/src/picodefs.h"
...
static char * doAddProperties( const char * str )
{
    ...
    //haspitch = 1; hasspeed = 1; hasvol = 1;
...
tts_result TtsEngine::synthesizeText( const char * text, int8_t * buffer, size_t bufferSize, void * userdata )
{
    ...
    //pico_Char * local_text = "<pitch level='90'><speed level='105'> \"hello world\" </speed></pitch>";
    ...
    if ( (strncmp(text, "<speak", 6) == 0) || (strncmp(text, "<?xml", 5) == 0) ) {
		...
		local_text = (pico_Char *) doAddProperties( parsed_text );
                //local_text = "<pitch level='90'><speed level='105'> "\hello world\" </speed></pitch>";
		if (!local_text) {
