...
#ifdef picolangdir
const char * PICO_LINGWARE_PATH             = picolangdir "/system/tts/lang_pico/";
#else
...
int main(int argc, const char *argv[]) {
    char * wavefile = "/data/local/tts.wav";
    char * lang = "en-US";
    ...
    fprintf(stderr,"pico2wave started,wavefile:/data/local/tts.wav\n"); 
    /*	
	poptContext optCon; 
	...
	*/
    ...
    if((ret = pico_initialize( picoMemArea, PICO_MEM_SIZE, &picoSystem ))) {
        ...
        fprintf(stderr, "Cannot initialize picotts (%i): %s\n", ret, outMessage);
        goto terminate;
    }
    fprintf(stderr, "picotts initialized(%i)\n", ret);
...
terminate:
    ...
    printf("Wavefile successfully Created\n");
    exit(ret);
}
